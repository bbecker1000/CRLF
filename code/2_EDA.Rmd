...
Comparing cover data: ground vs. interpolated

```{r}
# plots for data comparisons across all sites
ggplot(data = between_year_data) + 
  geom_smooth(aes(x = BRDYEAR, y = mean_ground_sub), color = "salmon", se = FALSE) +
  geom_smooth(aes(x = BRDYEAR, y = mean_interpolated_sub), color = "purple", se = FALSE)

ggplot(data = between_year_data) + 
  geom_smooth(aes(x = BRDYEAR, y = mean_ground_emerg), se = FALSE) +
  geom_smooth(aes(x = BRDYEAR, y = mean_interpolated_emerg), se = FALSE)

ggplot(data = between_year_data) + 
  geom_smooth(aes(x = BRDYEAR, y = mean_ground_open_water), se = FALSE) +
  geom_smooth(aes(x = BRDYEAR, y = mean_interpolated_open_water), se = FALSE)

# facet wrap by site
ggplot(data = between_year_data) + 
  geom_smooth(aes(x = BRDYEAR, y = mean_ground_sub), se = FALSE) +
  geom_smooth(aes(x = BRDYEAR, y = mean_interpolated_sub), se = FALSE) +
